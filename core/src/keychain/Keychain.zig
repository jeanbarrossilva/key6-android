...
pub const Key = struct {
    ...
    pub const Credential = struct {
        /// 256-bit sequence generated randomly by a CSPRNG, used to encrypt the
        /// password.
        key: [key_len]u8,

        /// Random bytes for producing different ciphertexts when encrypting two
        /// equal paswords. This is an input for encryption and, afterward,
        /// decryption.
        iv: [iv_len]u8,
...
/// Maximum amount of consecutive characters in the main password of a keychain.
...
pub fn unlock(self: *Self, io: std.Io, main_password: ?[]const u8) !void {
    ...
    std.crypto.pwhash.argon2.strVerify(
        ...
        _: {
            ...
            break :_ Error.TooManyUnlockAttempts;
        } else {
...
const test_main_password = "appleseed";
const test_key_password = "password123";
var test_csprng = std.Random.DefaultCsprng.init(
    [_]u8{0} ** std.Random.DefaultCsprng.secret_seed_length,
);
var test_prng = std.Random.DefaultPrng.init(0);

test "Key.validate(): errors if ID isn't a v7 UUID" {
    var key = initTestKey();
...
test "init(): main password isn't in hash" {
    ...
    try std.testing.expectEqual(
        null,
        std.mem.indexOf(u8, keychain.main_password_hash, test_main_password),
    );
}
...
test "init(): main password isn't discarded" {
    ...
    defer keychain.deinit();
    try std.testing.expectEqual("appleseed", test_main_password);
}
...
test "unlock(): unlocks if given the correct main password" {
    ...
    defer keychain.deinit();
    try keychain.unlock(std.testing.io, test_main_password);
}
...
test "storeKey(): key's password isn't stored in plaintext" {
    ...
    defer keychain.deinit();
    const template_key = initTestKey();
...
test "readPassword(): reads password of stored key" {
    ...
    try std.testing.expectEqualSlices(u8, test_key_password, read_password);
}
...
fn disableAutoLock(keychain: *Self) void {
