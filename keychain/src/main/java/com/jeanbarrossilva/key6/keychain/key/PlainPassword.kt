...
value class PlainPassword
  private constructor(
    private val backingBuffer: CharBuffer
  ) : CharSequence {
      ...
      WITH_DIACRITICS {
        override val subset =
          charArrayOf(
            ...
            'ä', 'å', 'æ', 'B', 'b', 'C', 'Ç', 'c', 'ç', 'D', 'Ð', 'd', 'ð',
            'E', 'È', 'É', 'Ê', 'Ë', 'e', 'è', 'é', 'ê', 'ë', 'F', 'f', 'G',
            'g', 'H', 'h', 'I', 'Ì', 'Í', 'Î', 'Ï', 'i', 'ì', 'í', 'î', 'ï',
            'J', 'j', 'K', 'k', 'L', 'l', 'M', 'm', 'N', 'Ñ', 'n', 'ñ', 'O',
            'Ò', 'Ó', 'Ô', 'Õ', 'Ö', 'Ø', 'o', 'ò', 'ó', 'ô', 'õ', 'ö', 'ø',
            'P', 'p', 'Q', 'q', 'R', 'r', 'S', 's', 'T', 't', 'U', 'Ù', 'Ú',
            'Û', 'Ü', 'u', 'ù', 'ú', 'û', 'ü', 'V', 'v', 'W', 'w', 'X', 'x',
            'Y', 'Ý', 'y', 'ý', 'ÿ', 'Z', 'z', 'Þ', 'þ'
          )
      };
     ...
     * requirement as per the HOTP and the TOTP RFCs. For the official
     * references, see the sections below of these RFCs:
     *
    ...
    sealed class TOTPException(
       ...
       */
      class LowEntropy internal constructor(
        val length: Int
      ) : TOTPException(
          "TOTP length should be in the [6, 8] range (was $length)."
        )

      /**
       * Thrown if the current time is lesser than zero; this denotes that such
       * time is before the Unix epoch (Jan 1, 1970, 00:00), which is
       * prohibited.
       *
       * @property currentTime Negative time passed into the TOTP generator.
       */
      class PreUnixEpochTime internal constructor(
        val currentTime: Duration
      ) : TOTPException("Current time cannot be before the Unix epoch.")

      /**
       * Thrown if the seed passed into the [generateTOTP] function has less
       * than 128 bits (16 bytes); such a seed is deemed too short in the RFC
       * and is prohibited as input to the algorithm.
       *
       * @property size Actual size of the given seed.
       */
      class ShortSeed internal constructor(
        val size: Int
      ) : TOTPException(
          "TOTP seed must contain at least 16 bytes (128 bits); given one " +
            "had $size."
        )
    }
     ...
     *
     * This operation is **unsafe** because this password gets duplicated, and
     * its contents are not moved to its clone.
     */
     ...
     */
    internal fun encode() =
      // 'Charset.encode(CharBuffer)' may search for the UTF-16 encoder, but
      // there will be no character to encode in case this password is empty. We
      // avoid that unnecessary work here.
      if (isEmpty()) {
        EMPTY_BYTE_ARRAY
      } else {
        ...
        if (encodedPassword.size > encodedPasswordLength) {
          encodedPassword =
            encodedPassword.sliceArray(0..<encodedPasswordLength)
        }
    ...
    companion object {
       ...
       * between such HOTP and the desired length *n* equals to the HOTP with
       * its last *n* digits, where 6 ≤ *n* ≤ 8, with the integer of this array
       * at *n* - `HOTP_LENGTH_RECOMMENDATION.last` +
      ...
      /**
       * Minimum amount of bytes that a key passed as input into [generateTOTP]
       * is required to contain, as per the TOTP RFC.
       */
       ...
       * Attempting to generate a TOTP with a length outside of this range in
       * Key6 will throw an exception.
       *
       * @see generateTOTP
      ...
      /**
       * Instantiates a plain password with the given contents. Each character
       * in the array is transferred to the password, such that the array will
       ...
       * resulted from encoding the characters of the original password. Each
       * byte is transferred from the array to the password, with the array
       * being zeroed by the time this function returns.
       *
       ...
       * @param allowsSymbols Whether non-alphanumeric characters may be
       *   included.
      ...
      internal fun generate(
        ...
        val alphabet = CharArray(alphabetSize)
        System.arraycopy(
          /* src = */ letters.subset,
          /* srcPos = */ 0,
          /* dest = */ alphabet,
       ...
       * alongside a regular, plain password defined by the user; this provides
       * an extra layer of security, allowing for the user to authenticate its
       * identity. Such authentication tends to be secure because, often,
       * generator programs are NIST-compliant, and the user owns the instance
       * of the generator program (here, Key6) by which the TOTP was generated.
       *
       * The usual user-facing flow until reaching the point of generating a
       * TOTP is the following:
       *
       ...
       * into the [seed] parameter. This function will, then, generate a TOTP
       * with the specified length, valid for the amount of time in the [step]
       * since the current time; past this duration plus the validation window
       * of the site, the TOTP expires, and, once MFA is enabled, a new one must
       * be generated for the user to authenticate.
       *
       * ## References
       *
       ...
       *   being thrown.
       * @param currentTime Time passed since the last Unix epoch, from which
       *   the step will be counted. Zero denotes Jan 1, 1970, 00:00; times
       *   prior to that (i.e., negative) are prohibited.
       ...
       *   9]]. Due to the low entropy that would result from lengths past
       *   either ends of the range, this method treats the RFC's recommendation
       *   on the length as a requirement: 6 ≤ [length] ≤ 8, defaulting to 6.
       */
      @JvmStatic
      ...
      internal fun generateTOTP(
        ...
        // This is where the implementation of the HOTP algorithm begins. The
        // TOTP algorithm simply imposes a constraint as to what the HOTP
        // counter parameter is: the amount of steps that the interval between
        // the Unix epoch and the current time comprises, floored.
        ...
        val truncationStartOffset =
          hash.last().toInt() and
            // 00000F (hexadecimal) = 00001111 (binary) = 15 (decimal). The
            // offset being the last nibble of the last byte of the
            // to-be-truncated TOTP restricts it to [0, 15].
            0x0f
        var totpAsInt =
          hash[truncationStartOffset].toInt() and Byte.MAX_VALUE.toInt()
        for (
        offset in (truncationStartOffset + 1)..(truncationStartOffset + 3)
        ) {
          totpAsInt =
 ...
 */
private fun ByteBuffer.discard() =
  discard {
    if (hasArray()) {
      array().discard()
    }
 ...
 */
private fun CharBuffer.discard() =
  discard {
    if (hasArray()) {
      array().discard()
    }
