...
abstract class Keychain {
  ...
  suspend fun store(
    ...
    val encryptedPassword =
      Base64.encode(encryptPassword(plainPassword, cipherNonce, passphrase))
    passphrase.fill(0)
    val key =
      Key(
        id,
  ...
  private suspend fun derivePassphraseFromMainPassword(
    ...
    val requestedMainPasswordAsArray = requestMainPassword().toCharArray()
    val sizeInBits = 256
    val spec =
      PBEKeySpec(
  ...
  private fun encryptPassword(
    ...
    val tagLengthInBits = 128
    val cipherModeSpec = GCMParameterSpec(tagLengthInBits, nonce)
